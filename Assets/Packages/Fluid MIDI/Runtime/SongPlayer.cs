...
namespace FluidMidi
{
    public class SongPlayer : MonoBehaviour
    {
        ...
        public ToggleInt Loop { get {return loop;} set {loop = value;}}
        [SerializeField]
        ...
        public void Play(int startTick)
        {
            ...
            if (!IsPlaying)
            {
                Api.Player.Seek(playerPtr, startTick);
                Api.Player.Play(playerPtr);
                unloadDelay = unloadOnStop.Value;
                IsPaused = false;
            }
        ...
        public void SetLoop(ToggleInt newLoop)
        {
            loop = newLoop;
            if (loop.Enabled)
            {
            ...
            {
                Api.Player.SetLoop(playerPtr,0);
            }
        }
        ...
        void OnEnable()
        {
            ...
            if (songPath.Length > 0)
            {
                ...
                {
                    Logger.Log("FluidMIDI: Song file missing: " + songPath);
                }
            }
            else
            {
                Logger.LogError("FluidMIDI: No song specified");
            }
            if (loop.Enabled)
            {
                Api.Player.SetLoop(playerPtr, -1);
