...
namespace FluidMidi
{
    public class Synthesizer : MonoBehaviour
    {
        ...
        {
            string oldPath = soundFont.path;
            ///print("Old path: "+oldPath);
            soundFont.path = path;
            LoadStreamingAsset(oldPath);
            RemoveReference();
            AddReference();
        }
        public bool Exists()
        {
            ...
            {
                string path = Application.streamingAssetsPath + soundFont.path;
                path = path.Replace('/','\\');
                ///print(path+" exists: "+File.Exists(path));
                return (File.Exists(path));
            }
            return false;
        }
        void LoadStreamingAsset(string oldPath)
        {
            ...
            if (soundFontPath.Length > 0 && Api.Misc.IsSoundFont(soundFontPath) == 0)
            {
                ...
                soundFont.SetFullPath(oldPath);
            }
        }
        void OnValidate()
        {
            LoadStreamingAsset(string.Empty);
        }
